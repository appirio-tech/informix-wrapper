...
JDBCConn.prototype.connect = function(callback) {
    ...
    self._log('Trying to connect with ' + self._config.timeout + ' ms timeout', 'debug');
...
JDBCConn.prototype.execute = function() {
    ...
    if (self._sql.trim().toLowerCase().indexOf('insert') === 0 ||
        self._sql.trim().toLowerCase().indexOf('update') === 0 ||
        self._sql.trim().toLowerCase().indexOf('delete') === 0) {
